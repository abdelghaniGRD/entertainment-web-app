...
function BookMarked() {
  ...
  const allbookmarks = data.filter((card) => {
    return card.isBookmarked;
  });

  const tvseries = allbookmarks.filter((card) => {
    return card.category === "TV Series";
  });
  const movies = allbookmarks.filter((card) => {
  ...
  return (
    <>
      <main>
        <Search
          title={"Search for BookMarked Shows"}
          setSearchQuery={setSearchQuery}
        />
        {searchQuery === "" ? (
          <>
            <p className="bookmarked">BookMarked Movies</p>
